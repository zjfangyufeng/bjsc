...
//      title = activity.getString(R.string.contents_contact);
  ...
  public static void setQRCode(final Context c, final ImageView iv, final String url){
    new Thread(new Runnable() {
      @Override
      public void run() {
        try {
          final Bitmap bitmap = getInstance(c,url).encodeAsBitmap();
          if(bitmap!=null){
  ...
  public static QRCodeEncoder getInstance(Context c,String url){
    ...
    try {
      qrCodeEncoder = new QRCodeEncoder(c, intent, 180, false);
    } catch (WriterException e) {
      e.printStackTrace();
    }
